            ...
            modalContent={
                <>
                    ...
                    <Button
                        className="metaplex-button-jumbo d-flex wallet-modal-btn"
                        onClick={async () => {
                            console.debug(`Selecting nearWalletAdapter name: ${nearWalletAdapter.name}`);
                            await select(nearWalletAdapter.name);
                            try {
                                console.debug(nearWalletAdapter);
...
export const WalletProvider: FC<WalletProviderProps> = ({
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const [localName, setLocalName] = useLocalStorageStringState(localStorageKey);
    ...
    const webWalletAdapter = useMemo(() => new WebWalletAdapter(), []);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (
                    wallet?.adapter === webWalletAdapter &&
                    webWalletAdapter?.chain &&
                    webWalletAdapter?.label &&
                    webWalletAdapter?.secretKey
                ) {
                    await adapter.connect(
                        webWalletAdapter.chain,
                        webWalletAdapter.label,
                        intArrayToString(webWalletAdapter.secretKey)
                    );
                } else {
                    await adapter.connect();
                }
    ...
    const setCredentials = useCallback(
        async (chain: string, label: string, privateKey: Uint8Array) => {
            ...
            if (adapter.name.includes('WebWallet')) {
                ...
                webWalletAdapter.setCredentials(chain, label, intArrayToString(privateKey));
            }
        },
        [name, adapter]
    );
    ...
    const connect = useCallback(
        async (chain?: string, label?: string, privateKey?: Uint8Array, force?: boolean, wltName?: WalletName) => {
            if (force && wltName && adapter && adapter.name.includes('WebWallet') && chain && label && privateKey) {
                setName(wltName);
                return await adapter.connect(chain, label, intArrayToString(privateKey));
            }
            ...
            setConnecting(true);
            try {
                privateKey && (await adapter.connect(chain, label, intArrayToString(privateKey)));
            } catch (error: any) {
                disconnect();
                setName(null);
            }
