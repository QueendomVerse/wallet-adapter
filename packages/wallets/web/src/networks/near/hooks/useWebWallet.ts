...
//     return Buffer.from(decodeBs58(pubKey)).toString('hex');
//   };
...
export interface NearAccount {
    ...
    displayAllNFT: (accountId: string) => Promise<Record<string, unknown>[] | undefined>;
...
export const useAccount = (privateKey: string, network?: string): NearAccount => {
    ...
    const displayAllNFT = async (accountId: string): Promise<Record<string, unknown>[] | undefined> => {
        const acct: Account | undefined = await account();
        if (!acct) return;
...
export interface WalletContextState {
    ...
    signAllTransactions: SolanaSignerWalletAdapterProps['signAllTransactions'] | undefined;
