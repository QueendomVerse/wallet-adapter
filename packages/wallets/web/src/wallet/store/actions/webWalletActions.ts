...
export const thunkCreateWallet =
  ...
  export const thunkRestoreWallet =
    (
      wallet: DbWallet, password: string
    ): ThunkAction<void, RootState, unknown, Action<string>> =>
    // async (dispatch, getState) => {
    async dispatch => {
...
export const thunkCreateTransaction =
  ...
  async (dispatch, getState) => {
    ...
    let result: any
    try {
      result = await sendTransaction(getChainProp(chain).ticker, keypair, toAddress, amount);
...
export const thunkMintNft =
  ...
  async (dispatch, getState) => {
    ...
    //   return null;
    // }
    console.error('func: thunkMintNft', chain, label)
    console.dir(keypair)
