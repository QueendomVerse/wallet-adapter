...
export async function getFiat() {
  const url = `https://api.coingecko.com/api/v3/simple/price?ids=near&vs_currencies=usd,eur,cny`;
  const controller = new AbortController();
    ...
    .then(data => {
      return {
        near: {
          ...
          eur: data.near.eur,
          cny: data.near.cny,
          last_updated_at: new Date().getTime(), // CoinGecko does not provide a timestamp, so we use the current time
        },
      };
    })) as {
    near: {
