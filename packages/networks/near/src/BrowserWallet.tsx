...
import { ChainNetworks, SolanaPublicKey, SolanaTransaction, WalletError } from '@mindblox-wallet-adapter/base';
...
abstract class BrowserWalletAdapter extends Emitter {
    abstract get publicKey(): NearPublicKey | null;
...
export class BrowserWallet extends React.Component<Props, State, BrowserWalletAdapter> {
    ...
    public wallets: BrowserWallet[] | null;
    ...
    _signWithKey = async (pubKey: NearPublicKey, data: Uint8Array): Promise<Signature> => {
        if (!this.connected) {
            throw new WalletError('Wallet not connected');
        }
        if (!this.selected) {
