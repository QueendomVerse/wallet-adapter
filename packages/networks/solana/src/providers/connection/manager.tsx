...
import { asyncEnsureRpcConnection } from '@mindblox-wallet-adapter/base';
...
export interface NetworkProps {
    endpoint: string;
    commitmentOrConfig?: Commitment | ConnectionContextState | undefined;
}
...
export interface ConnectProps {
    ...
    error: string | null;
    connection: SolanaConnection | undefined;
    establishConnection: (props: EstablishConnectionProps) => Promise<void>;
    clearConnection: () => void;
}

// export const ConnectionContext = createContext<SolanaConnection>(null!);

// const useShareablePreviousConnectionState = () => {
//   const [prevConnection, setPrevConnection] = useState<SolanaConnection | undefined>();
...
export const connectionManager = (): ConnectProps => {
    ...
    // const [toConnection, setToConnection] = useState<SolanaConnection | undefined>(prevConnection);
    const [toConnection, setToConnection] = useState<SolanaConnection | undefined>();

    const waitForOpenConnection = (connection: SolanaConnection): Promise<SolanaConnection> => {
    ...
    const handleConnection = useCallback(async (props: EstablishConnectionProps) => {
        ...
        if (props.network) {
            ...
            // return await waitForOpenConnection(new SolanaConnection(props.network.endpoint, {
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (result) {
                        ...
                        resolve(
                            `Solana SolanaConnection(v: ${apiVersion}) established at block ${blockHeight} ticking at ${slot}`
                        );
                    } else {
