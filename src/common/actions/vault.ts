...
export async function getSafetyDepositBox(
  ...
  if (!vault || !tokenMint) return;

  // console.warn('func: getSafetyDepositBox')
  console.debug("tokenMint", tokenMint);
  const vaultProgramId = programIds().vault;
...
export async function activateVault(
  ...
  const fractionMintAuthorityResult = await findProgramAddress(
    [
      Buffer.from(VAULT_PREFIX),
  ...
  const fractionMintAuthority =
    fractionMintAuthorityResult &&
    fractionMintAuthorityResult.length > 0 &&
    fractionMintAuthorityResult[0];
  ...
  const keys = [
    ...
    },
    {
      pubkey: fractionMintAuthority
        ? toPublicKey(fractionMintAuthority)
        : emptyKey,
      isSigner: false,
      isWritable: false,
    },
...
export async function combineVault(
  ...
  const burnAuthorityResult = await findProgramAddress(
    [
      Buffer.from(VAULT_PREFIX),
      toPublicKey(vaultProgramId).toBuffer(),
      toPublicKey(vault).toBuffer(),
  ...
  const burnAuthority =
    burnAuthorityResult &&
    burnAuthorityResult.length > 0 &&
    burnAuthorityResult[0];
  ...
  const keys = [
    ...
    {
      pubkey: burnAuthority ? toPublicKey(burnAuthority) : emptyKey,
      isSigner: false,
      isWritable: false,
    },
    {
