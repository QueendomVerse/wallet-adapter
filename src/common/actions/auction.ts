...
export async function createAuction(
  ...
  // console.warn('func: createAuction')
  ...
  const auctionKey: StringPublicKey = result[0];

  const extendedAuction = await getAuctionExtended({
    auctionProgramId,
  ...
  const keys = [
    ...
    {
      pubkey: extendedAuction ? toPublicKey(extendedAuction) : emptyKey,
      isSigner: false,
      isWritable: true,
    },
    {
...
export async function placeBid(
  ...
  const auctionKeyResult = await findProgramAddress(
    [
      Buffer.from(AUCTION_PREFIX),
      toPublicKey(auctionProgramId).toBuffer(),
      toPublicKey(resource).toBuffer(),
  ...
  if (!auctionKeyResult || auctionKeyResult.length < 1) return;
  const auctionKey: StringPublicKey = auctionKeyResult[0];
  ...
  const bidderMetaKeyResult = await findProgramAddress(
    [
      Buffer.from(AUCTION_PREFIX),
      toPublicKey(auctionProgramId).toBuffer(),
  ...
  if (!bidderMetaKeyResult || bidderMetaKeyResult.length < 1) return;
  const bidderMetaKey: StringPublicKey = bidderMetaKeyResult[0];

  let bidderPotTokenAccount: PublicKey = emptyKey;
  if (!bidderPotTokenPubkey && bidderPotKey) {
    const bidderPotTokenAccountResult = await findProgramAddress(
      [
        Buffer.from(AUCTION_PREFIX),
    ...
    if (!bidderPotTokenAccountResult || bidderPotTokenAccountResult.length < 1)
      return;
    bidderPotTokenAccount = toPublicKey(bidderPotTokenAccountResult[0]);
  } else if (bidderPotTokenPubkey) {
    console.debug("auction: bidderPotTokenPubkey", bidderPotTokenPubkey);
    bidderPotTokenAccount = toPublicKey(bidderPotTokenPubkey);
    // bidderPotTokenAccount = toPublicKey('5e8PD4CoHsBjPc3VjswfF13P2m8qLr2VnFbdvvBpEYS1');
  }

  console.info(`bidderPotTokenPubkey: ${bidderPotTokenPubkey}`);
  const extendedAuction = await getAuctionExtended({
    auctionProgramId,
    resource,
  });
...
export async function cancelBid(
  ...
  const auctionKey = auctionKeyResult[0];
  ...
  if (!bidderMetaKeyResult || bidderMetaKeyResult.length < 1) return;
  const bidderMetaKey = bidderMetaKeyResult[0];
