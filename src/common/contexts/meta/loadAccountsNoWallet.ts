...
import { MAX_ALLOWED_CREATOR_SIZE } from "../../models";
import {
  // getEdition,
  Metadata,
  // MAX_NAME_LENGTH,
...
export const loadAccountsNoWallet = async (
  connection: Connection,
  ...
  const queryAuctionsFromAuctionManagers = async (
    ...
    const auctionData = await getMultipleAccounts(
      connection,
      [...auctionIds, ...auctionExtendedKeys].filter((k): k is string => !!k)
    );
...
//   connection: Connection,
//   allowedCreatorsByCreator: Record<
//     string,
...
export const loadAuction = async (
  ...
  const bidRedemptionData = await getMultipleAccounts(
    connection,
    bidderRedemptionIds.filter((d): d is string => !!d),
    "single"
  );
