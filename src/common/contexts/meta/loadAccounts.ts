...
import { MAX_ALLOWED_CREATOR_SIZE, TokenAccount } from "../../models";
...
export const loadMetadataForUsers = async (
  ...
  console.info("--------->Pulling metadata for user.");
  ...
  for (let i = 0; i < userTokenAccounts.length; i++) {
    if (userTokenAccounts[i].info.amount.toNumber() == 1) {
      if (2 + currBatch.length > MULTIPLE_ACCOUNT_BATCH_SIZE) {
        ...
        currBatch = currBatch.concat(newAdd.filter((a): a is string => !!a));
      }
    }
  }
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    if (accounts) {
      ...
      console.error("------->Failed to pull batch", i, "skipping");
    }
  }

  console.info("------> Pulling master editions for user");
  currBatch = [];
  batches = [];
  if (!editions || editions.length < 1) return;
  const nonNullEditions = editions.filter((e): e is string => !!e);
  for (let i = 0; i < editions.length; i++) {
    if (1 + currBatch.length > MULTIPLE_ACCOUNT_BATCH_SIZE) {
      batches.push(currBatch);
      currBatch = [];
    } else if (state.editions[nonNullEditions[i]]) {
      currBatch.push(state.editions[nonNullEditions[i]].info.parent);
    }
  }
  ...
  await postProcessMetadata({ ...state, allowedCreatorsByCreator });

  console.info("-------->User metadata processing complete.");
  return state;
};
...
export const loadAccounts = async (
  ...
  const queryAuctionsFromCache = async () => {
    ...
    const readyMetadata = auctionCaches.reduce((memo, auctionCache) => {
      const setMetadata = auctionCache.info.metadata?.map(
        async (metadataKey) => {
  ...
  const queryAuctionsFromAuctionManagers = async (
    parsedAccounts: ParsedAccount<AuctionManagerV2>[]
  ) => {
    ...
    const auctionData = await getMultipleAccounts(
      connection,
      [...auctionIds, ...auctionExtendedKeys].filter((d): d is string => !!d)
    );
  ...
  const queryVaultsForAuctionManagers = async (
    auctionManagers: ParsedAccount<AuctionManagerV2>[]
  ) => {
...
const queryCreators = async (
  ...
  const response = await getProgramAccounts(connection, METAPLEX_ID, {
    filters: [
      {
        dataSize: MAX_ALLOWED_CREATOR_SIZE,
      },
    ],
  });
...
export const loadPrizeTrackingTickets = async (
  connection: Connection,
  auctionManager: ParsedAccount<AuctionManagerV2>,
  ...
  const prizeTrackingTicketsResponse = await getMultipleAccounts(
    connection,
    prizeTrackingKeys.filter((k): k is string => !!k),
    "single"
  );
...
export const loadMetaDataAndEditionsForCreators = async (
  ...
  const loadMetadata = () =>
    pullMetadataByCreators(connection, allowedCreatorsByCreator);
...
const pullEditions = async (
  ...
  const updateState = makeSetter(state);
  console.info("Pulling editions for optimized metadata");
  ...
  for (const metadata of state.metadata) {
    let editionKey: StringPublicKey | undefined;
    if (metadata?.info?.editionNonce === null && metadata?.info?.mint) {
    ...
    editionKey && setOf100MetadataEditionKeys.push(editionKey);
...
export const loadArtwork = async (
  ...
  console.debug(`loadArtwork, key: ${key}`);

  let metaResponse: MetaResponse | undefined = undefined;
  try {
    metaResponse = await getMultipleAccounts(connection, [key], "single");
  } catch (error) {
  ...
  if (!metaResponse || !metaResponse.array || metaResponse.array.length < 1) {
  ...
  await initMetadata(metadataAccount, allowedCreatorsByCreator, updateState);
...
// const { wallet, connect, connected, select, publicKey} = useWallet();
export const loadAuction = async (
  ...
  const updateState = makeSetter(state);
  const forEachAccount = processingAccounts(updateState);
  console.debug(`auctionManager: ${auctionManager}`);
  ...
  try {
    ...
    console.error(`Failed to load Auction Manager Account: ${err}`);
  }
  ...
  const bidRedemptionData = await getMultipleAccounts(
    connection,
    bidderRedemptionIds.filter((id): id is string => !!id),
    "single"
  );
...
export const loadMetadataAndEditionsBySafetyDepositBoxes = async (
  ...
  const metadataData = await getMultipleAccounts(
    connection,
    metadataKeys.filter((k): k is string => !!k),
    "single"
  );
  ...
  const readyMetadata = metadata?.map((m) =>
    initMetadata(m, allowedCreatorsByCreator, updateState)
  );
...
const pullMetadataByCreators = async (
  ...
  console.info("pulling optimized nfts");

  const allowedCreators = Object.values(allowedCreatorsByCreator);

  const additionalPromises: Promise<MetaState>[] = [];
  for (const creator of allowedCreators) {
...
export const processingAccounts =
  ...
  async (accounts: AccountAndPubkey[]) => {
    if (!accounts) {
      console.error("No processing accounts found!");
      return;
    }
...
export const metadataByMintUpdater = async (
  ...
  if (isMetadataPartOfStore(metadata, state.allowedCreatorsByCreator)) {
    await metadata?.info.init();
...
export const initMetadata = async (
  metadata: ParsedAccount<Metadata>,
  allowedCreators: Record<string, ParsedAccount<WhitelistedCreator>>,
  setter: UpdateStateValueFunc
) => {
  if (isMetadataPartOfStore(metadata, allowedCreators)) {
    await metadata?.info.init();
