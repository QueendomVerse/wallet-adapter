...
import { chunks, sleep } from "../../utils/utils";
    ...
    for (let i = 0; i < signedTxns.length; i++) {
      ...
      try {
        ...
        if (breakEarlyObject.breakEarly) {
          ...
          throw new Error(`Died on: '${breakEarlyObject.i}'`);
        }
      }
    }
  }
...
export const sendTransactionWithRetry = async (
  ...
  console.debug(
    `sendTransactionWithRetry; includesFeePayer: ${includesFeePayer}`
  );
  ...
  console.debug(
    `signedTransaction2; recentBlockhash: ${transaction.recentBlockhash}`
  );
...
export const sendSignedTransaction = async ({
  ...
  console.debug(
    `sendSignedTransaction; feePayer: ${signedTransaction.feePayer}`
  );
  console.debug(
    `sendSignedTransaction; instructions: ${signedTransaction.instructions}`
  );
  console.debug(
    `sendSignedTransaction; nonceInfo: ${signedTransaction.nonceInfo}`
  );
  console.debug(
    `sendSignedTransaction; recentBlockhash: ${signedTransaction.recentBlockhash}`
  );
  console.debug(
    `sendSignedTransaction; signature: ${signedTransaction.signature}`
  );
  ...
  console.error(`Started awaiting confirmation for: '${txid}'`);

  let done = false;
  const startSendRawTransaction = async () => {
    // done = true;
    while (!done && getUnixTs() - startTime < timeout) {
      (await asyncEnsureRpcConnection(connection)).sendRawTransaction(
        rawTransaction,
        {
          skipPreflight: true,
        }
  ...
  try {
    ...
    if (typeof err === "object" && "InstructionError" in err) {
      throw new Error(
        `InstructionError: ${getTransactionInstructionError(err)}`
      );
    }
    if (err.timeout) {
      throw new Error("Timed out awaiting confirmation on transaction");
    ...
    try {
      ...
      throw new Error(`SimulateTransactionError: ${e}`);
    }
...
const simulateTransaction = async (
  ...
  const res = await (
    ...
    // @ts-ignore
    ._rpcRequest("simulateTransaction", args);
  if (res.error) {
