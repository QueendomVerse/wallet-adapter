...
import { isHex, isBase58 } from "../../../../../utils";
import { fetchPublicKeys } from "./helper-api";

export const isValidAccount = async (account: string) => {
  if (
    isValidNearName(account) ||
    (await getImplicitId(account)) ||
    (isHex(account) && account.length == 64)
  ) {
  ...
  const ids = publicKeys
    ...
    .map(({ public_key }) => {
      ...
      const id = Buffer.from(
        bs58.decode(public_key?.replace("ed25519:", ""))
      ).toString("hex");
...
//@TODO use noble-ed25519 or something to create a validity check
export const isImplicitAddress = async (address: string) => {
  return address;
};
