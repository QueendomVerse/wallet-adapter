...
export const actionsObjToHash = async ({
  ...
  // console.error('account_id', accountId);
...
export const getSign = (json: { message: string }) => {
  ...
  if (!recoverySeedPhrase) return console.info("not SeedPhras");
  if (!json.message) return console.info("not message");
  const { secretKey } = nearSeedPhrase.parseSeedPhrase(recoverySeedPhrase);
