...
export const connectionManager = (): ConnectProps => {
  ...
  const handleConnection = useCallback(
    async (props: EstablishConnectionProps) => {
      ...
      if (
        props.connection &&
        (await (
          await asyncEnsureRpcConnection(props.connection)
        ).getBlockHeight())
      ) {
      ...
      if (props.network) {
        // const nodeWsUri: string | undefined = publicRuntimeConfig.publicSolanaRpcHost ?? undefined;
        console.info(
          `Attempting to establish Solana network connection via: ${props.network.endpoint} ...`
        );
  ...
  const initConnection = (
    props: EstablishConnectionProps,
    timeout: number = 2000
  ): Promise<string> => {
        ...
        try {
          ...
          if (result) {
            const { apiVersion, slot } = (
              await (
                await asyncEnsureRpcConnection(result)
              ).getBlockProduction()
    ...
    async (props: EstablishConnectionProps) => {
      ...
      if (!props.connection && !props.network) {
        throw new ConnectionError(
          "Unable to establish a connection without network parameters"
        );
      }
