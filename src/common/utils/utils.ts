...
export const holdUrHorses = async (waitTime: number) => {
  console.debug(`Waiting for ${waitTime} ...`);
...
export const asyncEnsureRpcConnection = async (
  conn: Connection,
  delay: number = 1000,
  ...
  console.debug(
    `connection web socket state: ${
      //@ts-ignore
      conn._rpcWebSocketConnected ? "connected" : "disconnected"
    }`
  );
  //@ts-ignore
  while (count < maxTries && !conn._rpcWebSocketConnected) {
    ...
    try {
      ...
      throw Error(`Failed to establish RPC connection: ${error}`);
    }
    count++;
  }
  //@ts-ignore
  if (!conn._rpcWebSocketConnected) {
    throw Error("Unable to establish RPC connection!");
  }
  return conn;
};

//@TODO figure out how to do the same check for non-async connections
export const ensureRpcConnection = (conn: Connection) => {
  conn
    ?.getVersion()
    .then((version) => {
      console.info(`solana version: ${version["solana-core"]}`);
    })
    .catch((err) => {
      throw Error(`Solana Connection Error: ${err}`);
    });
};
