...
import { Chains as lChains } from "../../contexts/connection";
...
export const decryptWallet = async (wallet: lWallet, password: string) => {
  ...
  try {
    ...
    throw new Error(
      `Failed to decrypt ${wallet.chain} ${wallet.label} wallet ${wallet.pubKey} seed phrase!`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to decrypt ${wallet.chain} ${wallet.label} wallet ${wallet.pubKey} private key!`
    );
  }
