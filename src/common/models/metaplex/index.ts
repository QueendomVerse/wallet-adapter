...
export const MAX_ALLOWED_CREATOR_SIZE = 2 + 32 + 10;
...
export class AuctionManager {
  ...
  instance: ParsedAccount<AuctionManagerV2>;
  ...
  constructor(args: {
    ...
    this.safetyDepositBoxesExpected = new BN(args.vault.info.tokenTypeCount);
    ...
    for (let i = 0; i < this.numWinners.toNumber(); i++) {
      ...
      this.safetyDepositConfigs?.forEach((s) => {
        ...
        if (amount.gt(new BN(0))) {
          ...
          // console.info('safetyDeposit')
          // console.dir(safetyDeposit);
          const metadata =
            safetyDeposit && metadataByMint[safetyDeposit.info.tokenMint];
          metadata &&
            newWinnerArr.push({
              metadata,
...
export class AuctionManagerV2 {
  ...
  constructor(args: {
    ...
    // console.debug(`AuctionManagerV2.this.key: ${this.key}`);
    this.store = args.store;
    ...
    // console.debug(`AuctionManagerV2.auction: ${auction}`);
...
export class SetAllowedCreatorArgs {
  instruction = 9;
  activated: boolean;
...
export const decodeAllowedCreator = (buffer: Buffer) => {
  return deserializeUnchecked(
    SCHEMA,
    WhitelistedCreator,
    buffer
...
export const AllowedCreatorParser: AccountParser = (
  pubkey: StringPublicKey,
  account: AccountInfo<Buffer>
) => ({
  pubkey,
  account,
  info: decodeAllowedCreator(account.data),
});
...
export const decodeAuctionManager = (buffer: Buffer): AuctionManagerV2 => {
  return deserializeUnchecked(SCHEMA, AuctionManagerV2, buffer);
};
...
export class WhitelistedCreator {
  key: MetaplexKey = MetaplexKey.AllowedCreatorV1;
  id: string;
...
export async function getAuctionKeys(
  vault: string
): Promise<{ auctionKey: string; auctionManagerKey: string } | undefined> {
  if (!vault) return;
  const PROGRAM_IDS = programIds();
  ...
  if (!result || result.length < 1) return;
  const auctionKey = result[0];

  const auctionManagerKey = await getAuctionManagerKey(auctionKey);
  if (!auctionManagerKey) return;
...
export async function getBidRedemption(
  ...
  if (!auctionKey || bidMetadata || bidMetadata.length < 1) return;
...
export async function getBidderKeys(
  auctionKey: string,
  bidder: string
): Promise<{ bidMetadata: string; bidRedemption: string } | undefined> {
  if (!auctionKey || bidder) return;
...
export async function getOriginalAuthority(
  ...
  const result = await findProgramAddress(
    [
      Buffer.from(METAPLEX_PREFIX),
      toPublicKey(auctionKey).toBuffer(),
      metadata ? toPublicKey(metadata).toBuffer() : emptyKey.toBuffer(),
    ],
    toPublicKey(PROGRAM_IDS.metaplex)
  );
...
export const isCreatorPartOfTheStore = async (
  ...
  if (!creatorAddress || !pubkey) return;

  const creatorKeyInStore = await getAllowedCreator(creatorAddress, store);

  return creatorKeyInStore === pubkey;
};

export async function getAllowedCreator(
...
export async function getSafetyDepositConfig(
  ...
  if (!auctionManager || !safetyDeposit) return;
  const PROGRAM_IDS = programIds();
  const store = PROGRAM_IDS.store;
  if (!store) {
    throw new Error("Store not initialized");
