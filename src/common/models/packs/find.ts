...
export async function getProgramAuthority(): Promise<StringPublicKey> {
  ...
  const programAddresses = await findProgramAddress(
    [
  ...
  if (!programAddresses || programAddresses.length < 1) {
    throw new Error("Found no program addresses!");
  }
  return programAddresses[0];
}
